...
def load_data():
        ...
        5. Upsert post vectors into the vector database:
            - store post in qdrant data base , thier id and vector value
        ...
        7. Upsert user profile vectors into the vector database:
            - store user interaction vector on database by according to user di
        ...
        try:
            ...
            for index, row in posts_df.iterrows():
                ...
                Post.objects.create(content_id=row['content_id'], title=row['title'], content=row['content'], post_type=row['post_type'] , post_type_format=row['post_type_format'])
            ...
            for idx, row in posts_df.iterrows():
                ...
                point = PointStruct(id=row['content_id'], vector=vector, payload={"post_type": row['post_type'],"post_type_format":row['post_type_format']})
                points.append(point)
